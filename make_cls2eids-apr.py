...
if __name__ == '__main__':
    ...
    parser.add_argument('-dataset', default='../data/apr', help='path to dataset folder')
    ...
    parser.add_argument('-path_num_cls2eids', default='num_cls2eids-apr.txt')
        ...
        for file in os.listdir(os.path.join(args.dataset, args.path_expand_result)):
            ...
            with open(os.path.join(args.dataset, args.path_expand_result, file), encoding='utf-8') as f:
                for i, line in enumerate(f):
                    ...
                    if 10 <= i < 20 and cls_name in ['usstates', 'parties']:
                        temp = line.strip().split('\t')
                        cls2eids[cls_name].add(int(temp[0]))
            ...
            with open(os.path.join(args.dataset, args.path_expand_result, file), encoding='utf-8') as f:
                for i, line in enumerate(f):
                    if i >= 102:
                        temp = line.strip().split('\t')
                        eid = int(temp[0])
                        if eid not in gt_eids[cls_name]:
                            neg_nums[cls_name][(i-102) // 5] += 1

                        if 112 <= i < 117:
                            if cls_name in ['usstates', 'parties']:
    ...
    # for i in range(20):
    #     num = 0
    #     temp = '[' + str(100 + i*5 + 1) + '-' + str(100 + (i+1)*5) + ']'
    #     for cls_name in classes:
